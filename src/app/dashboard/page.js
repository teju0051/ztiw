...
export default function ZenTechDashboard() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Failed to load local stickers due to strict browser settings.");
    }
    ...
    if (profile && !profileError) {
      if (profile.ban_status && profile.ban_status !== "none") {
        if (profile.ban_status === "temporary" && new Date() >= new Date(profile.ban_until)) {
          ...
          Swal.fire({ title: "Access Revoked", text: "There is an error at our end please login again", icon: "error", allowOutsideClick: false }).then(() => router.push("/login"));
          return;
        }
      }
      ...
      Swal.fire({ title: "Profile Access Denied", html: `Login succeeded, but we couldn't find your role in the system.`, icon: "error", confirmButtonText: "Return to Login", allowOutsideClick: false }).then(() => { supabase.auth.signOut(); router.push("/login"); });
    }
  };
  ...
  const handleAvatarUpload = async (event) => {
    ...
    if (uploadError) { setIsUploadingAvatar(false); return Swal.fire("Upload Failed", uploadError.message, "error"); }
  ...
  const handleGroupAvatarUpload = async (event) => {
    ...
    if (uploadError) { setIsUploadingGroupAvatar(false); return Swal.fire("Upload Failed", uploadError.message, "error"); }
  ...
  const getDivisionStyle = (div) => {
    ...
    if(div === 'QA & Operations') return 'bg-rose-50 text-rose-700 border-rose-200';
  ...
  const getChatBubbleStyle = (team, isMe, showColors) => {
    if (isMe) return "bg-indigo-600 border-indigo-700 text-white rounded-[20px] rounded-tr-sm";
    
    // Standard styling for isolated team channels
    if (!showColors) return "bg-white border-slate-200 text-slate-800 rounded-[20px] rounded-tl-sm";

    // Tinted bubbles for global/admin/1-1 networking
    if (team === 'Core AI & Backend') return "bg-purple-50 border-purple-200 text-slate-800 rounded-[20px] rounded-tl-sm";
    if (team === 'Tools & Integrations') return "bg-blue-50 border-blue-200 text-slate-800 rounded-[20px] rounded-tl-sm";
    if (team === 'QA & Operations') return "bg-rose-50 border-rose-200 text-slate-800 rounded-[20px] rounded-tl-sm";
    if (team === 'System Administration') return "bg-slate-100 border-slate-300 text-slate-800 rounded-[20px] rounded-tl-sm";
  ...
  // ==========================================
  // CHAT: CHANNELS & 1-1 DIRECT MESSAGES
  // ==========================================
  const fetchUserChannels = async (profile, dirMap) => {
    ...
    let directMessages = [];
    try {
      const { data } = await supabase.from('direct_messages').select('*').or(`user1_id.eq.${profile.id},user2_id.eq.${profile.id}`);
      if (data) directMessages = data;
    } catch (e) {
      console.warn("Direct Messages table missing.");
    }

    const getAvatar = (chName) => {
      const meta = channelMeta?.find(m => m.channel_name === chName);
      return meta?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(chName)}&background=e0e7ff&color=4f46e5`; 
    };
    ...
    baseChannels.push({ id: "All Teams", label: "All Teams Network", avatar_url: getAvatar("All Teams"), memberIds: allUserIds, lead: "System Administration", canEdit: profile.role === 'admin', isDirect: false });

    if (profile.role === 'admin' || profile.role === 'team_lead') {
      baseChannels.push({ id: "Admin", label: profile.role === 'admin' ? "Admin Hub" : "Admin Network", avatar_url: getAvatar("Admin"), memberIds: allUserIds.filter(id => dirMap[id].role === 'admin' || dirMap[id].role === 'team_lead'), lead: "System Administration", canEdit: profile.role === 'admin', isDirect: false });
    }
    ...
    const buildTeamChannel = (teamName) => {
      ...
      return { id: teamName, label: teamName, avatar_url: getAvatar(teamName), memberIds: memberIds, lead: leadProfile ? leadProfile.full_name : "Unassigned", canEdit: profile.role === 'admin' || (profile.role === 'team_lead' && myTeamName === teamName), isDirect: false };
    };

    if (profile.role === 'admin') {
       const t1 = buildTeamChannel("Core AI & Backend"); const t2 = buildTeamChannel("Tools & Integrations"); const t3 = buildTeamChannel("QA & Operations");
       if (t1) baseChannels.push(t1); if (t2) baseChannels.push(t2); if (t3) baseChannels.push(t3);
    ...
    if (directMessages.length > 0) {
      directMessages.forEach(dm => {
        const otherUserId = dm.user1_id === profile.id ? dm.user2_id : dm.user1_id;
        const otherUser = dirMap[otherUserId];
        if (otherUser) {
          baseChannels.push({
            id: dm.channel_id,
            label: otherUser.full_name,
            avatar_url: otherUser.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(otherUser.full_name)}&background=f3f4f6&color=64748b`,
  ...
  const handleStartDirectMessage = async (targetUserId) => {
    try {
      ...
      const { data } = await supabase.from('direct_messages').select('*').eq('channel_id', channelId).single();
      
      if (!data) {
        const { error } = await supabase.from('direct_messages').insert([{ channel_id: channelId, user1_id: userProfile.id, user2_id: targetUserId }]);
        if (error) {
          Swal.fire("Silent Block Detected", "The direct_messages insertion was rejected by Supabase. Error: " + error.message, "error");
          return;
        }
      }
      ...
      Swal.fire("System Error", "Failed to start 1-on-1 chat: " + err.message, "error");
    }
  };
  ...
  const fetchChatPreviews = async () => {
    ...
    for (const ch of availableChannels) {
      ...
      if (latest && latest.length > 0) {
         ...
         recentMessagesForDashboard.push({ id: msgId, channelId: ch.id, channelLabel: ch.label, channelAvatar: ch.avatar_url, senderName, senderAvatar, text, time, created_at: msg.created_at, unreadCount });
         ...
         if (!isInitialFetch.current && msg.sender_id !== userProfile.id && !notifiedIdsRef.current.has(msgId)) {
            ...
            if (shouldNotify && Notification.permission === "granted") new Notification(`${ch.label}`, { body: `${senderName}: ${text}`, icon: senderAvatar });
            notifiedIdsRef.current.add(msgId);
         } else if (isInitialFetch.current) { notifiedIdsRef.current.add(msgId); }
      }
  ...
  const fetchChatMessages = async () => {
    ...
    let queryCols = "id, message, media_url, media_type, created_at, edited_at, reply_to, is_pinned, sender_id, read_by, profiles:sender_id(full_name, role, avatar_url)";
    let res = await supabase.from("chats").select(queryCols).eq("channel", activeChatChannel).order("created_at", { ascending: true });
    
    if (res.error) {
       queryCols = "id, message, media_url, media_type, created_at, sender_id, read_by, profiles:sender_id(full_name, role, avatar_url)";
       res = await supabase.from("chats").select(queryCols).eq("channel", activeChatChannel).order("created_at", { ascending: true });
    }
    ...
    if (!error && data) {
      ...
      const pinned = data.slice().reverse().find(m => m.is_pinned);
  ...
  useEffect(() => {
    if (!isUserScrolling && chatEndRef.current) chatEndRef.current.scrollIntoView({ behavior: "smooth" });
  }, [chatMessages]);

  // ==========================================
  // CHAT: NEW FEATURES LOGIC 
  // ==========================================
  
  const allUsersList = Object.values(globalDirectory).map(p => ({
    ...p,
    staff_id: p.email ? p.email.split('@')[0] : `ZT-${p.id.substring(0, 8).toUpperCase()}`
  }));
  ...
  const handleAddSticker = (e) => {
    ...
    reader.onload = (event) => {
      ...
      try { localStorage.setItem("zentech_stickers", JSON.stringify(updatedStickers)); } catch(e){}
  ...
  const sendSticker = async (base64String) => {
    try {
      setShowStickerPicker(false);
      const { error } = await supabase.from("chats").insert([{ channel: activeChatChannel, sender_id: userProfile.id, media_url: base64String, media_type: 'sticker' }]);
      if (error) Swal.fire("Sticker Error", error.message, "error");
      else fetchChatMessages();
    } catch (err) {
      Swal.fire("Code Error", err.message, "error");
    }
  };
  ...
  const renderMessageText = (text) => {
    ...
    return parts.map((part, i) => {
      if (part.match(/^https?:\/\//)) {
        return <a key={i} href={part} onClick={(e) => handleLinkWarning(e, part)} className="text-indigo-500 font-bold hover:underline transition-all cursor-pointer break-all">{part}</a>;
      }
  ...
  const handlePinMessage = async (msgId, currentPinState) => {
    try {
      const { data, error } = await supabase.from("chats").update({ is_pinned: !currentPinState }).eq("id", msgId).select();
      
      if (error) {
         Swal.fire("Error", "Error pinning: " + error.message, "error");
      } else if (!data || data.length === 0) {
         Swal.fire("Silent Block Detected", "Supabase is silently blocking this pin request. Ensure you ran the SQL command to disable Row Level Security.", "warning");
      } else {
         fetchChatMessages();
      }
    } catch (err) {
      Swal.fire("Code Error", "Failed to execute Pin function: " + err.message, "error");
    }
  };
  ...
  const handleSendChatMessage = async () => {
    ...
    try {
      ...
      if (pastedImage) {
        ...
        const { error: uploadError } = await supabase.storage.from("chat_media").upload(fileName, pastedImage);
        if (!uploadError) {
      ...
      if (editingMessage) {
        const { data, error } = await supabase.from("chats").update({ message: chatInput.trim(), edited_at: new Date().toISOString() }).eq("id", editingMessage.id).select();
        
        if (error) {
          Swal.fire("Database Error", error.message, "error");
        } else if (!data || data.length === 0) {
          Swal.fire("Silent Block Detected", "Supabase RLS is silently blocking your Edit request. Please run the SQL command.", "warning");
        } else {
        ...
        const { data: insertedData, error } = await supabase.from("chats").insert([payload]).select().single();

        if (error) {
          Swal.fire("Database Schema Error", "The `reply_to` column does not exist. Run the SQL script. \n\n Error: " + error.message, "error");
        } else if (insertedData) {
          try { await fetch('/api/webhooks/chat-notification', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ record: insertedData }) }); } catch (err) {}
        }
      }
      ...
      Swal.fire("Code Error", "Failed to execute Send function: " + err.message, "error");
    } finally {
      setIsSendingChat(false);
    }
  };
  ...
  const handleChatMediaUpload = async (event) => {
    ...
    if (!isImage && !isVideo) { Swal.fire("Invalid Format", "Only Image and Video files are allowed.", "error"); if (chatMediaInputRef.current) chatMediaInputRef.current.value = ""; return; }
    ...
    try {
      ...
      const { data: insertedMediaData, error: dbError } = await supabase.from("chats").insert([{ channel: activeChatChannel, sender_id: userProfile.id, message: null, media_url: publicUrlData.publicUrl, media_type: mediaType }]).select().single();
        
      if (chatMediaInputRef.current) chatMediaInputRef.current.value = "";
      setIsUserScrolling(false); fetchChatMessages(); fetchChatPreviews();
      
      if (!dbError && insertedMediaData) {
        try { await fetch('/api/webhooks/chat-notification', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ record: insertedMediaData }) }); } catch (err) {}
      }
    } catch (err) {
       Swal.fire("Code Error", "Failed to upload media: " + err.message, "error");
    } finally {
       setIsSendingChat(false);
    }
  };
  ...
  const showGroupInfo = () => {
    ...
    window.viewFullscreenAvatar = (url) => { Swal.fire({ imageUrl: url, imageAlt: 'Group Avatar', showConfirmButton: false, width: 'auto', background: 'transparent', backdrop: `rgba(0,0,0,0.8)` }); };
    ...
    Swal.fire({
      ...
      `,
      showConfirmButton: false, showCloseButton: true, background: 'transparent', padding: '0', width: '450px'
    });
  };

  const handleViewStaffTasks = async (staffId, staffName, ztId) => {
    Swal.fire({ title: 'Retrieving Telemetry...', allowOutsideClick: false, didOpen: () => Swal.showLoading(), background: '#ffffff' });
    ...
    if (!error) {
      ...
      if (!staffTasks || staffTasks.length === 0) taskHtml += `<div style="text-align: center; padding: 20px; color: #64748b; font-size: 13px; font-weight: 600;">No active or completed tasks assigned to this operative.</div>`;
      else {
          ...
          taskHtml += `
            <div style="padding: 12px; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px;">
              ...
              <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <span style="font-size: 11px; color: #94a3b8; font-weight: 600;">${new Date(t.created_at).toLocaleDateString()}</span>
              </div>
            </div>
          `;
        });
      ...
      Swal.fire({
        ...
        html: taskHtml, confirmButtonText: 'Close Window', confirmButtonColor: '#4f46e5', width: '500px', background: '#f8fafc'
      });
  ...
  const handleBanStaff = async (staff) => {
    if (staff.ban_status !== 'none') {
      if (staff.revoke_count >= 1) return Swal.fire("Revocation Blocked", "This staff member has exhausted their 1 revoke chance.", "error");
      Swal.fire({ title: `Revoke Ban for ${staff.full_name}?`, text: "You have 1 chance to revoke a ban per staff member. After this, any future ban will be permanent.", icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, Unban', confirmButtonColor: '#10b981', background: '#ffffff' }).then(async (result) => {
        if (result.isConfirmed) {
          ...
          if (error || !updatedData || updatedData.length === 0) Swal.fire('Action Blocked', `Supabase security prevented this action.`, 'error');
          else { await supabase.from("activity_logs").insert([{ actor_name: userProfile.full_name, actor_role: userProfile.role, action_description: `Revoked ban for ${staff.full_name}` }]); Swal.fire('Restored', 'Staff access has been reinstated.', 'success'); fetchAllStaff(); }
        }
      });
      return;
    }
    ...
    const { value: formValues } = await Swal.fire({
      ...
      `,
      focusConfirm: false, showCancelButton: true, confirmButtonText: 'Enforce Ban', confirmButtonColor: '#e11d48', background: '#ffffff',
      preConfirm: () => {
        const type = document.getElementById('tempBan').checked ? 'temporary' : 'permanent'; const reason = document.getElementById('banReason').value;
        if (!reason) Swal.showValidationMessage('A reason for the ban is required.'); return { type, reason };
      }
    });
    ...
    if (formValues) {
      ...
      if (error || !updatedData || updatedData.length === 0) Swal.fire('Action Blocked', `Supabase security prevented the ban.`, 'error');
      else { await supabase.from("activity_logs").insert([{ actor_name: userProfile.full_name, actor_role: userProfile.role, action_description: `Issued a ${formValues.type} ban to ${staff.full_name}. Reason: ${formValues.reason}` }]); Swal.fire('Banned', `User blocked.`, 'success'); fetchAllStaff(); }
    }
  };
  ...
  const fetchNotifications = async (userId) => {
    ...
    if (!error && data && data.length > 0) {
      Swal.fire({ title: "⚠️ Operational Notification", html: `<div style="text-align: left; background: #f8fafc; padding: 16px; border-radius: 8px; border-left: 4px solid #4f46e5; font-weight: 600; color: #334155;">${data[0].message}</div>`, icon: "info", confirmButtonText: "Acknowledge", confirmButtonColor: "#4f46e5", background: '#ffffff' }).then(async () => { await supabase.from("notifications").update({ is_read: true }).eq("id", data[0].id); });
    }
  };
  ...
  const handleAssignToTeam = async (memberId, memberName) => {
    Swal.fire({
      ...
      input: "select", inputOptions: { "Core AI & Backend": "Core AI & Backend", "Tools & Integrations": "Tools & Integrations", "QA & Operations": "QA & Operations" }, showCancelButton: true, confirmButtonText: "Deploy Engineer", confirmButtonColor: '#4f46e5', background: '#ffffff'
    }).then(async (result) => {
      if (result.isConfirmed && result.value) {
        ...
        if (teamData) {
          ...
          if (!error) { setUnassignedEngineers((prev) => prev.filter((m) => m.id !== memberId)); fetchAllTeamsWithMembers(); if (userProfile.role === 'admin') fetchAllStaff(); await fetchGlobalDirectory(); await supabase.from("activity_logs").insert([{ actor_name: userProfile.full_name, actor_role: userProfile.role, action_description: `Deployed ${memberName} to ${result.value}` }]); Swal.fire("Assigned!", `${memberName} has been deployed.`, "success"); }
        }
      }
    });
  };
  ...
  const handleAssignTaskToMember = async (memberId, memberName) => {
    ...
    const { data: previousTasks, error: fetchError } = await supabase.from('tasks').select('title, status, created_at').eq('assigned_to', memberId).order('created_at', { ascending: false }).limit(6);
    if (fetchError) { Swal.fire('Error', 'Could not fetch operative telemetry.', 'error'); return; }
    ...
    Swal.fire({
      ...
      `,
      width: '950px', padding: '40px', background: '#ffffff', showCancelButton: true, buttonsStyling: true, confirmButtonText: 'Assign Task <i class="fa-solid fa-paper-plane" style="margin-left: 8px;"></i>', cancelButtonText: 'Cancel', confirmButtonColor: '#4f46e5', cancelButtonColor: '#94a3b8',
      preConfirm: () => {
        const desc = document.getElementById("task-desc").value; const priority = document.getElementById("task-priority").value;
        if (!desc.trim()) { Swal.showValidationMessage("A detailed task description is required."); return false; } return { desc: desc.trim(), priority };
      }
    }).then(async (result) => {
      if (result.isConfirmed && result.value) {
        const { desc, priority } = result.value; const taskTitle = `[${priority}] ${desc}`;
        ...
        if (!taskError) {
          ...
          Swal.fire({ title: "Task Dispatched!", html: `<p style="font-size: 14px; font-weight: 500; color: #64748b;">The directive has been securely transmitted to ${memberName}.</p>`, icon: "success", confirmButtonColor: "#4f46e5", background: '#ffffff', confirmButtonText: "Acknowledged" }); fetchTasks();
        } else Swal.fire("Error", "Failed to dispatch task: " + taskError.message, "error");
      }
    });
  };
  ...
  const handleAdminDispatchDirective = async () => {
    const { value: formValues } = await Swal.fire({
      title: "Announcement",
      html: `
        <input id="dir-title" class="swal2-input" placeholder="Enter High-Priority Task Title..." style="width: 85%; font-size: 14px; border-radius: 8px; border: 1px solid #cbd5e1;">
        <select id="dir-team" class="swal2-input" style="width: 85%; font-size: 14px; border-radius: 8px; border: 1px solid #cbd5e1;">
      ...
      `,
      focusConfirm: false, showCancelButton: true, confirmButtonText: "Dispatch Directive", confirmButtonColor: "#f59e0b", background: '#ffffff',
      preConfirm: () => {
        const title = document.getElementById("dir-title").value; const team = document.getElementById("dir-team").value; const file = document.getElementById("dir-file").files[0];
        if (!title) Swal.showValidationMessage("Title is required"); return { title, team, file };
      },
    });
    ...
    if (formValues) {
      ...
      if (formValues.file) {
        Swal.fire({ title: "Uploading Document...", allowOutsideClick: false, didOpen: () => Swal.showLoading(), background: '#ffffff' });
  ...
  const fetchTasks = async () => {
    ...
    if (!error && data) {
      const formattedTasks = data.map((task) => ({ id: task.id, title: task.title, status: task.status, team_id: task.team_id, is_admin_directive: task.is_admin_directive, file_url: task.file_url, assigned_to: task.assigned_to, team: task.teams?.name || "Assigned Operations", assignedToName: task.profiles?.full_name || "Unassigned", assignedByName: task.assigned_by_name || "Team Lead", adminFeedback: task.admin_feedback || "None" }));
  ...
  const handleFileUpload = async (event) => {
    ...
    if (!dbError) { await supabase.from("activity_logs").insert([{ actor_name: userProfile.full_name, actor_role: userProfile.role, action_description: `Submitted a Bi-Weekly Report: "${file.name}"` }]); Swal.fire("Submitted!", "Bi-Weekly report successfully uploaded for Admin review.", "success"); fetchReports(); }
  };
  ...
  const handleApproveReport = async (reportId, leadName, teamName, leadId) => {
    ...
    if (!error) { await supabase.from("notifications").insert([{ user_id: leadId, message: `✅ Admin approved your bi-weekly report for ${teamName}.` }]); await supabase.from("activity_logs").insert([{ actor_name: userProfile.full_name, actor_role: userProfile.role, action_description: `Approved report from ${teamName}` }]); Swal.fire("Approved", "Report marked as approved.", "success"); fetchReports(); }
  };

  const handleRejectReport = async (reportId, leadName, teamName, leadId) => {
    Swal.fire({ title: "Reject Report", html: `<input type="text" id="report-reject-reason" class="swal2-input" placeholder="Enter reason for rejection...">`, confirmButtonText: "Reject Report", confirmButtonColor: "#e11d48", background: "#ffffff", preConfirm: () => document.getElementById("report-reject-reason").value }).then(async (result) => {
      if (result.isConfirmed && result.value) {
        const { error } = await supabase.from("team_reports").update({ status: "rejected", admin_feedback: result.value }).eq("id", reportId);
        if (!error) { await supabase.from("notifications").insert([{ user_id: leadId, message: `❌ Admin rejected your bi-weekly report. Reason: ${result.value}` }]); await supabase.from("activity_logs").insert([{ actor_name: userProfile.full_name, actor_role: userProfile.role, action_description: `Rejected report from ${teamName}` }]); Swal.fire("Rejected", "Report rejected and feedback logged.", "info"); fetchReports(); }
      }
    });
  };
  ...
  const handleRejectCompletion = async (taskId) => {
    Swal.fire({ title: "Reject Task Completion", html: `<input type="text" id="reject-reason" class="swal2-input" placeholder="Reason for rejection...">`, confirmButtonText: "Confirm Rejection", confirmButtonColor: "#e11d48", background: "#ffffff", preConfirm: () => document.getElementById("reject-reason").value }).then(async (result) => {
      if (result.isConfirmed && result.value) {
      ...
      <li>
        ...
        >
          <div className="relative flex items-center justify-center">
            <i className={`${icon} text-[1.1rem] transition-colors duration-300 ${isActive ? "text-white" : "text-slate-400 group-hover:text-indigo-600"}`}></i>
          ...
          {!isSidebarCollapsed && <span className={`truncate ${!isActive ? "group-hover:text-indigo-700" : ""}`}>{label}</span>}
          {!isSidebarCollapsed && badgeCount > 0 && (
  ...
  const activeChObj = availableChannels.find(c => c.id === activeChatChannel);
  // Determine if we should show the color-coded UI for the current channel
  const showRoleBadgeAndColors = activeChatChannel === 'All Teams' || activeChatChannel === 'Admin' || activeChObj?.isDirect;
  ...
  return (
    <>
      ...
      <div className="flex h-screen overflow-hidden text-slate-900 antialiased font-sans bg-slate-50 relative w-full">
        ...
        {activeTab === "dashboard" && (
          <div className="absolute top-7 right-52 z-50 animate-in fade-in duration-700 hidden sm:flex items-center gap-3 bg-white px-4 py-2.5 rounded-full shadow-[0_4px_16px_rgba(0,0,0,0.03)] border border-slate-200 hover:shadow-md hover:-translate-y-0.5 transition-all cursor-default">
        ...
        {/* PREMIUM PROFILE FLOAT */}
        {activeTab === "dashboard" && (
          <div className="absolute top-6 right-8 z-50 animate-in fade-in duration-500">
            <div 
              className="relative group cursor-pointer bg-white rounded-full pl-5 pr-2 py-2 flex items-center gap-3 shadow-sm border border-slate-200 transition-all hover:shadow-md hover:-translate-y-0.5" 
              onClick={() => avatarInputRef.current.click()} 
              title="Change Avatar"
            >
               <div className="text-right flex flex-col justify-center">
                 <p className="text-[13px] font-black text-slate-900 leading-tight">{userProfile.full_name}</p>
                 <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{userProfile.role.replace("_", " ")}</p>
               </div>
        ...
        <aside className={`${isSidebarCollapsed ? "w-24" : "w-[280px]"} bg-white border-r border-slate-200 shadow-none flex flex-col justify-between flex-shrink-0 z-20 whitespace-nowrap transition-all duration-300 ease-in-out`}>
          ...
          <div className="p-6">
            <button onClick={handleLogout} className={`w-full flex items-center ${isSidebarCollapsed ? "justify-center" : "justify-start"} px-4 py-3 text-sm font-bold text-rose-600 bg-rose-50 border border-transparent hover:border-rose-200 hover:text-white hover:bg-rose-500 hover:shadow-sm rounded-xl transition-all duration-300`}>
        ...
        <main className="flex-1 flex flex-col h-screen overflow-hidden w-full relative">
          <div className="flex-1 overflow-y-auto p-4 sm:p-8 w-full custom-scrollbar">
            <div className="w-full h-full">
              ...
              {/* SECTION: DASHBOARD */}
              {activeTab === "dashboard" && (
                <div className="space-y-8 animate-in fade-in duration-500 mt-4 w-full">
                  <div>
                    <h1 className="text-[32px] font-black text-slate-900 tracking-tight">Dashboard Overview</h1>
                    <p className="text-slate-500 text-sm mt-1 font-medium">Welcome back. Here is your real-time telemetry.</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
                     <div className="bg-white rounded-[20px] shadow-sm hover:shadow-md p-6 flex flex-col justify-center border border-slate-200 hover:border-slate-300 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
                        <div className="flex items-center gap-4 mb-4">
                           <div className="w-14 h-14 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-600 text-2xl group-hover:scale-110 transition-transform"><i className="fa-solid fa-bars-progress"></i></div>
                           <div>
                              <p className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Total Tasks</p>
                              <h3 className="text-3xl font-black text-slate-900">{tasks.length}</h3>
                           </div>
                        </div>
                        <div className="flex items-center text-[11px] font-bold text-emerald-700 bg-emerald-50 w-fit px-3 py-1.5 rounded-lg border border-emerald-200">
                           <i className="fa-solid fa-arrow-trend-up mr-1.5"></i> Active Processing
                        </div>
                     </div>

                     <div className="bg-white rounded-[20px] shadow-sm hover:shadow-md p-6 flex flex-col justify-center border border-slate-200 hover:border-slate-300 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
                        <div className="flex items-center gap-4 mb-4">
                           <div className="w-14 h-14 rounded-2xl bg-purple-50 flex items-center justify-center text-purple-600 text-2xl group-hover:scale-110 transition-transform"><i className="fa-solid fa-clipboard-check"></i></div>
                           <div>
                              <p className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Pending Approvals</p>
                              <h3 className="text-3xl font-black text-slate-900">{tasks.filter((t) => t.status === "pending_completion_approval").length}</h3>
                           </div>
                        </div>
                        <div className="flex items-center text-[11px] font-bold text-purple-700 bg-purple-50 w-fit px-3 py-1.5 rounded-lg border border-purple-200">
                           <i className="fa-regular fa-clock mr-1.5"></i> Awaiting Review
                        </div>
                     </div>

                     <div className="bg-white rounded-[20px] shadow-sm hover:shadow-md p-6 flex flex-col justify-center border border-slate-200 hover:border-slate-300 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
                        <div className="flex items-center gap-4 mb-4">
                           <div className="w-14 h-14 rounded-2xl bg-rose-50 flex items-center justify-center text-rose-600 text-2xl group-hover:scale-110 transition-transform"><i className="fa-solid fa-bolt"></i></div>
                           <div>
                              <p className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Golden Directives</p>
                              <h3 className="text-3xl font-black text-slate-900">{tasks.filter((t) => t.is_admin_directive && t.status !== "completed").length}</h3>
                           </div>
                        </div>
                        <div className="flex items-center text-[11px] font-bold text-rose-700 bg-rose-50 w-fit px-3 py-1.5 rounded-lg border border-rose-200">
                           <i className="fa-solid fa-fire mr-1.5"></i> High Priority Action
                        </div>
                     </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 w-full">
                    <div className="bg-white rounded-[24px] shadow-sm hover:shadow-md p-8 border border-slate-200 flex flex-col transition-all duration-300">
                      <h3 className="text-xl font-extrabold text-slate-900 mb-8">{userProfile.role === "admin" ? "Division Task Distribution" : "Team Task Success Rate"}</h3>
                      <div className="flex flex-col sm:flex-row items-center gap-10 w-full justify-around flex-1">
                        {userProfile.role === "admin" ? (
                          <>
                            <div className="donut-chart shadow-sm border border-slate-100" style={{ background: adminConicGradient }}>
                              <div className="donut-hole shadow-sm"><span className="text-3xl font-black text-slate-900">{totalAdminTasks}</span></div>
                            </div>
                            <div className="flex flex-col space-y-4 bg-slate-50 p-5 rounded-2xl border border-slate-200 w-full sm:w-auto">
                              <div className="flex items-center text-sm font-bold text-slate-700"><span className="w-4 h-4 rounded-md shadow-sm bg-[#6366f1] mr-3"></span> Core AI & Backend ({payalTasks})</div>
                              <div className="flex items-center text-sm font-bold text-slate-700"><span className="w-4 h-4 rounded-md shadow-sm bg-[#3b82f6] mr-3"></span> Tools & Integrations ({sushantTasks})</div>
                              <div className="flex items-center text-sm font-bold text-slate-700"><span className="w-4 h-4 rounded-md shadow-sm bg-[#f43f5e] mr-3"></span> QA & Operations ({pratikTasks})</div>
                            </div>
                          </>
                        ) : (
                          <>
                            <div className="donut-chart shadow-sm border border-slate-100" style={{ background: leadConicGradient }}>
                              <div className="donut-hole shadow-sm"><span className="text-3xl font-black text-slate-900">{totalLeadTasks}</span></div>
                            </div>
                            <div className="flex flex-col space-y-4 bg-slate-50 p-5 rounded-2xl border border-slate-200 w-full sm:w-auto">
                              <div className="flex items-center text-sm font-bold text-slate-700"><span className="w-4 h-4 rounded-md shadow-sm bg-[#10b981] mr-3"></span> Success / Approved ({successTasks})</div>
                              <div className="flex items-center text-sm font-bold text-slate-700"><span className="w-4 h-4 rounded-md shadow-sm bg-[#f59e0b] mr-3"></span> In Progress ({pendingTasks})</div>
                              <div className="flex items-center text-sm font-bold text-slate-700"><span className="w-4 h-4 rounded-md shadow-sm bg-[#f43f5e] mr-3"></span> Failure / Rejected ({failTasks})</div>
                            </div>
                          </>
                        )}
                      </div>
                    ...
                    <div className="bg-white rounded-[24px] shadow-sm hover:shadow-md border border-slate-200 flex flex-col overflow-hidden h-[400px] transition-all duration-300 relative">
                      ...
                      <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-white">
                        <h3 className="text-xl font-extrabold text-slate-900 flex items-center">
                          <i className="fa-solid fa-bell mr-3 text-indigo-500"></i> Active Network Pings
                        </h3>
                      </div>
                      ...
                      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-slate-50">
                         {unreadDashboardMessages.length === 0 ? (
                           <div className="h-full flex flex-col items-center justify-center text-slate-400 p-8">
                             ...
                             <p className="text-sm font-bold">Network is silent. No unread messages.</p>
                           </div>
                         ) : (
                           <div className="space-y-3">
                             {unreadDashboardMessages.map((msg, idx) => (
                               <div key={idx} onClick={() => { setActiveChatChannel(msg.channelId); setActiveTab('chat'); }} className="bg-white border border-slate-200 hover:border-indigo-300 hover:shadow-sm p-4 rounded-2xl cursor-pointer transition-all duration-200 flex gap-4 items-center group">
                                  <div className="relative shrink-0">
                                    <img src={msg.channelAvatar} className="w-12 h-12 rounded-full object-cover shadow-sm border border-slate-200 group-hover:scale-105 transition-transform" />
                                  ...
                                  <div className="flex-1 min-w-0">
                                     <div className="flex justify-between items-baseline mb-1">
                                        <h4 className="text-sm font-extrabold text-slate-900 group-hover:text-indigo-600 transition-colors truncate">{msg.channelLabel}</h4>
                                        <span className="text-[10px] font-bold text-slate-500 ml-2 whitespace-nowrap bg-slate-50 border border-slate-200 px-2 py-0.5 rounded-md shadow-sm">{msg.time}</span>
                                     </div>
              ...
              {/* SECTION: MEETINGS / TEAM CONNECT */}
              {activeTab === "meetings" && (
                <div className="h-full animate-in fade-in duration-500 w-full flex flex-col items-center justify-center">
                  <h1 className="text-[32px] font-black text-slate-900 mb-4 tracking-tight">Team Connect</h1>
                  <p className="text-slate-500 mb-8 font-medium">Enter a secure channel name to initialize a WebRTC video link.</p>
                  <div className="bg-white rounded-[24px] shadow-sm border border-slate-200 p-8 flex flex-col items-center max-w-md w-full">
                    <i className="fa-solid fa-video text-5xl text-indigo-500 mb-6 drop-shadow-sm"></i>
                    <input type="text" id="meeting-room-input" placeholder="e.g. zentech-alpha" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 mb-6 text-center font-bold text-slate-900 focus:border-indigo-500 outline-none transition-colors" />
                    <button onClick={() => setActiveMeetingRoom(document.getElementById('meeting-room-input').value || 'general')} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl transition-all shadow-sm flex items-center justify-center gap-2">
                       Join Secure Feed <i className="fa-solid fa-arrow-right"></i>
                    </button>
                  </div>
                </div>
              )}
              ...
              {activeTab === "team" && (
                <div className="h-full flex flex-col space-y-6 animate-in fade-in duration-500 w-full">
                  <div>
                    <h1 className="text-[32px] font-black text-slate-900 tracking-tight">Team Management</h1>
                    <p className="text-slate-500 text-sm mt-1 font-medium">Manage operational deployments and engineer assignments.</p>
                  </div>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1 min-h-0 w-full">
                     {userProfile.role === "admin" && (
                       <div className="bg-white rounded-[24px] shadow-sm border border-slate-200 flex flex-col overflow-hidden">
                          <div className="p-6 border-b border-slate-100 bg-white">
                            <h3 className="text-lg font-black text-slate-900">Unassigned Personnel</h3>
                          </div>
                          <div className="overflow-y-auto p-4 custom-scrollbar bg-slate-50 flex-1">
                            {unassignedEngineers.length === 0 ? (
                               <div className="text-center text-slate-400 py-8 font-bold text-sm">No unassigned personnel.</div>
                            ) : (
                               unassignedEngineers.map(eng => (
                                 <div key={eng.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between mb-3">
                                    <span className="font-bold text-slate-900">{eng.full_name}</span>
                                    <button onClick={() => handleAssignToTeam(eng.id, eng.full_name)} className="text-xs bg-indigo-50 text-indigo-600 font-bold px-4 py-2 rounded-lg hover:bg-indigo-100 transition-colors border border-indigo-100 shadow-sm">Assign to Division</button>
                                 </div>
                               ))
                            )}
                          </div>
                     ...
                     <div className={`${userProfile.role !== 'admin' ? 'lg:col-span-2' : ''} bg-white rounded-[24px] shadow-sm border border-slate-200 flex flex-col overflow-hidden`}>
                        <div className="p-6 border-b border-slate-100 bg-white">
                          <h3 className="text-lg font-black text-slate-900">{userProfile.role === 'admin' ? 'Active Roster Overview' : 'My Division Personnel'}</h3>
                        </div>
                        <div className="overflow-y-auto p-4 custom-scrollbar bg-slate-50 flex-1">
                          {userProfile.role === 'admin' ? (
                             allTeamsData.map(team => (
                                <div key={team.id} className="mb-6">
                                  <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-3 px-2 flex items-center gap-2"><i className="fa-solid fa-users text-slate-300"></i> {team.name}</h4>
                                  {team.team_members && team.team_members.length > 0 ? team.team_members.map(tm => (
                                     <div key={tm.user_id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between mb-2">
                                        <span className="font-bold text-slate-900">{tm.profiles?.full_name}</span>
                                        <button onClick={() => handleAssignTaskToMember(tm.user_id, tm.profiles?.full_name)} className="text-xs bg-emerald-50 border border-emerald-100 text-emerald-600 font-bold px-4 py-2 rounded-lg hover:bg-emerald-100 transition-colors shadow-sm"><i className="fa-solid fa-plus mr-1"></i> Assign Task</button>
                                     </div>
                                  )) : <p className="text-xs font-bold text-slate-400 px-2 italic">No personnel deployed.</p>}
                                </div>
                             ))
                          ) : (
                             teamMembers.length > 0 ? teamMembers.map(tm => (
                               <div key={tm.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between mb-3">
                                  <span className="font-bold text-slate-900">{tm.name}</span>
                                  <button onClick={() => handleAssignTaskToMember(tm.id, tm.name)} className="text-xs bg-emerald-50 border border-emerald-100 text-emerald-600 font-bold px-4 py-2 rounded-lg hover:bg-emerald-100 transition-colors shadow-sm"><i className="fa-solid fa-plus mr-1"></i> Assign Task</button>
                               </div>
                             )) : <p className="text-sm font-bold text-slate-400 text-center py-8">Your division roster is empty.</p>
                          )}
                        </div>
                     </div>
                  </div>
                ...
                <div className="h-full flex gap-6 pb-2 animate-in fade-in duration-500 w-full relative">
                    ...
                    <div className="absolute inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
                      <div className="bg-white rounded-[24px] shadow-xl w-full max-w-md flex flex-col overflow-hidden border border-slate-200 animate-in fade-in zoom-in-95">
                        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                          <h3 className="font-black text-slate-900">Start Direct Message</h3>
                          <button onClick={() => setShowNewChatModal(false)} className="text-slate-400 hover:text-rose-500"><i className="fa-solid fa-xmark text-xl"></i></button>
                        </div>
                        <div className="p-4 border-b border-slate-100 relative">
                          <i className="fa-solid fa-search absolute left-7 top-1/2 -translate-y-1/2 text-slate-400"></i>
                          <input type="text" placeholder="Search Staff ID or Name..." value={chatSearchQuery} onChange={(e) => setChatSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium outline-none focus:border-indigo-400 transition-colors" />
                        </div>
                        <div className="overflow-y-auto max-h-[300px] p-2 custom-scrollbar">
                          {allUsersList.filter(s => s.id !== userProfile.id && (s.full_name.toLowerCase().includes(chatSearchQuery.toLowerCase()) || s.staff_id.toLowerCase().includes(chatSearchQuery.toLowerCase()))).length === 0 ? (
                            <p className="text-slate-400 text-center py-4 text-sm font-bold">No staff found.</p>
                          ) : (
                            allUsersList.filter(s => s.id !== userProfile.id && (s.full_name.toLowerCase().includes(chatSearchQuery.toLowerCase()) || s.staff_id.toLowerCase().includes(chatSearchQuery.toLowerCase()))).map(staff => (
                              <div key={staff.id} onClick={() => handleStartDirectMessage(staff.id)} className="flex items-center gap-3 p-3 hover:bg-slate-50 cursor-pointer rounded-xl transition-colors border border-transparent hover:border-slate-200">
                                <img src={staff.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(staff.full_name)}&background=f3f4f6&color=64748b`} className="w-10 h-10 rounded-full object-cover border border-slate-200" />
                                <div>
                                  <p className="font-bold text-slate-900 text-sm">{staff.full_name}</p>
                                  <p className="text-[10px] text-slate-400 font-mono uppercase tracking-widest">{staff.staff_id} • {staff.role.replace('_', ' ')}</p>
                                </div>
                              </div>
                            ))
                          )}
                  ...
                  <div className="w-[320px] bg-white rounded-[24px] shadow-sm border border-slate-200 flex flex-col overflow-hidden flex-shrink-0">
                    <div className="p-6 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
                      <h2 className="font-black text-slate-900 text-xl tracking-tight">Messages</h2>
                      <button onClick={() => setShowNewChatModal(true)} title="New 1-1 Chat" className="w-8 h-8 rounded-xl bg-white hover:bg-indigo-50 text-slate-500 hover:text-indigo-600 flex items-center justify-center transition-all border border-slate-200 shadow-sm">
                          ...
                          >
                            ...
                            <div className="flex flex-col overflow-hidden w-full">
                              <div className="flex justify-between items-center w-full">
                                <span className="font-extrabold text-slate-900 text-sm truncate">{ch.label}</span>
                                {preview?.time && <span className={`text-[10px] whitespace-nowrap ${preview.count > 0 && activeChatChannel !== ch.id ? 'text-indigo-600 font-black' : 'text-slate-400 font-bold'}`}>{preview.time}</span>}
                              </div>
                              <div className="flex justify-between items-center w-full mt-1">
                                <span className="text-xs text-slate-500 truncate pr-2 font-medium">
                                  {preview ? <span className="font-bold text-slate-700">{preview.sender}: </span> : ''}
                  ...
                  <div className="flex-1 bg-white rounded-[24px] shadow-sm border border-slate-200 flex flex-col overflow-hidden w-full relative">
                    <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center relative z-20 shadow-sm">
                      {activeChObj && (
                        <div className="flex items-center gap-4 cursor-pointer hover:bg-white p-2 rounded-2xl transition-colors w-full" onClick={showGroupInfo} title="View Group Info">
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="font-black text-slate-900 text-lg truncate">{activeChObj.label}</h3>
                            <p className="text-[11px] text-slate-500 font-bold uppercase tracking-widest truncate mt-0.5">
                               {activeChObj.isDirect ? "Direct Message" : activeChObj.memberIds.map(id => globalDirectory[id]?.full_name?.split(' ')[0]).filter(Boolean).join(', ')}
                            </p>
                          </div>
                        </div>
                      )}
                    ...
                    {pinnedMessage && (
                      <div onClick={() => scrollToMessage(pinnedMessage.id)} className="bg-indigo-50 border-b border-indigo-100 px-5 py-2.5 flex items-center justify-between cursor-pointer hover:bg-indigo-100 transition-colors z-10 shadow-sm animate-in slide-in-from-top-2">
                        <div className="flex items-center gap-3 overflow-hidden">
                          ...
                          <div className="flex flex-col truncate">
                            <span className="text-[10px] font-black text-indigo-700 uppercase tracking-widest">Pinned Message</span>
                            <span className="text-xs text-slate-700 font-medium truncate w-[300px] sm:w-[500px]">{pinnedMessage.message || "Media Attachment..."}</span>
                          </div>
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); handlePinMessage(pinnedMessage.id, true); }} className="text-indigo-300 hover:text-rose-500 p-2 rounded-lg hover:bg-white transition-colors shrink-0"><i className="fa-solid fa-xmark"></i></button>
                      </div>
                    )}

                    <div ref={chatContainerRef} onScroll={handleChatScroll} className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50 custom-scrollbar">
                      {chatMessages.length === 0 ? (
                        ...
                        chatMessages.map((msg) => {
                          ...
                          const isEditable = isMe && (new Date() - new Date(msg.created_at) < 2 * 60 * 60 * 1000); 
                          ...
                          const readByNames = (msg.read_by || []).filter(id => id !== msg.sender_id).map(id => globalDirectory[id]?.full_name?.split(' ')[0]).filter(Boolean);
                          const readByText = readByNames.length > 0 ? `Seen by ${readByNames.join(', ')}` : '';

                          return (
                            <div key={msg.id} id={`msg-${msg.id}`} className={`flex flex-col w-full ${isMe ? "items-end" : "items-start"} mb-6`}>
                              
                              <div className={`flex items-center gap-1 mb-1 ${isMe ? "justify-end mr-12" : "justify-start ml-12"} bg-white border border-slate-200 rounded-md shadow-sm px-1 py-0.5`}>
                                <button onClick={() => setReplyingToMessage(msg)} className="w-6 h-6 rounded hover:bg-slate-100 text-slate-500 flex items-center justify-center transition-colors" title="Reply"><i className="fa-solid fa-reply text-[10px]"></i></button>
                                <button onClick={() => handlePinMessage(msg.id, msg.is_pinned)} className={`w-6 h-6 rounded hover:bg-slate-100 flex items-center justify-center transition-colors ${msg.is_pinned ? 'text-indigo-600 bg-indigo-50' : 'text-slate-500'}`} title={msg.is_pinned ? "Unpin" : "Pin Message"}><i className="fa-solid fa-thumbtack text-[10px]"></i></button>
                                {isEditable && (
                                  <button onClick={() => { setEditingMessage(msg); setChatInput(msg.message); setTimeout(() => chatInputRef.current?.focus(), 50); }} className="w-6 h-6 rounded hover:bg-slate-100 text-slate-500 flex items-center justify-center transition-colors" title="Edit (Within 2 hrs)"><i className="fa-solid fa-pen text-[10px]"></i></button>
                                )}
                              </div>
                              ...
                              <div className={`flex gap-3 max-w-[75%] ${isMe ? "flex-row-reverse" : "flex-row"}`}>
                                ...
                                <div className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                                  <div className={`px-5 py-3 shadow-sm text-sm border ${getChatBubbleStyle(senderTeam, isMe, showRoleBadgeAndColors)}`}>
                                    
                                    {repliedMsg && (
                                      <div onClick={() => scrollToMessage(repliedMsg.id)} className={`mb-3 p-2.5 rounded-xl cursor-pointer border-l-4 transition-colors ${isMe ? 'bg-indigo-700/50 border-l-indigo-300 text-indigo-100 hover:bg-indigo-700' : 'bg-slate-50 border-l-indigo-500 text-slate-600 hover:bg-slate-100 border border-slate-200'}`}>
                                        <span className={`font-bold text-[10px] uppercase tracking-widest block mb-1 ${isMe ? 'text-indigo-200' : 'text-indigo-600'}`}>{repliedMsg.profiles?.full_name || "Unknown"}</span>
                                        <span className="text-xs truncate block max-w-[200px] opacity-90">{repliedMsg.message || "Media Attachment"}</span>
                                      </div>
                                    )}

                                    <div className={`flex justify-between items-baseline mb-2 gap-4 border-b ${isMe ? 'border-white/20' : 'border-slate-200'} pb-1.5`}>
                                      <div className="flex items-center gap-2">
                                        <span className={`block font-extrabold text-xs ${isMe ? "text-white" : (showRoleBadgeAndColors ? getChatNameColor(senderTeam) : "text-slate-900")}`}>
                                          {isMe ? "You" : (msg.profiles?.full_name || 'Unknown')}
                                        </span>
                                        
                                        {!isMe && senderTeam !== "Unassigned" && showRoleBadgeAndColors && (
                                          <span className={`text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded shadow-sm border ${getDivisionStyle(senderTeam)}`}>
                                            {senderTeam}
                                          </span>
                                        )}
                                      </div>
                                    ...
                                    {msg.media_type === 'sticker' && <img src={msg.media_url} className="w-28 h-28 object-contain my-2 drop-shadow-md" />}
                                    {msg.media_type === 'image' && <img src={msg.media_url} alt="Chat Upload" className="max-w-full h-auto rounded-xl mb-2 mt-2 shadow-sm border border-slate-200/50" style={{ maxHeight: "300px" }} />}
                                    {msg.media_type === 'video' && <video src={msg.media_url} controls className="max-w-full h-auto rounded-xl mb-2 mt-2 shadow-sm border border-slate-200/50" style={{ maxHeight: "300px" }} />}
                                    {msg.message && <p className="whitespace-pre-wrap leading-relaxed font-medium">{renderMessageText(msg.message)}</p>}
                                    
                                    {msg.edited_at && <span className={`block text-right text-[9px] italic font-bold mt-1 ${isMe ? 'text-indigo-300' : 'text-slate-400'}`}>Edited</span>}
                                    
                                    {isMe && readByText && (
                                      <div className="text-[10px] text-indigo-200 mt-2 text-right italic font-bold flex justify-end items-center gap-1">
                                        <i className="fa-solid fa-check-double"></i> {readByText}
                                      </div>
                                    )}
                                  </div>
                                </div>
                      ...
                      {showStickerPicker && (
                        <div className="bg-white border border-slate-200 p-4 rounded-2xl shadow-xl w-72 pointer-events-auto mb-2 self-start ml-2 flex flex-wrap gap-2 max-h-56 overflow-y-auto custom-scrollbar animate-in slide-in-from-bottom-2">
                          <div className="w-full flex justify-between items-center mb-3 border-b border-slate-100 pb-2">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Stickers Vault</span>
                            <button onClick={() => stickerInputRef.current.click()} className="text-[10px] bg-indigo-50 border border-indigo-100 text-indigo-600 px-2.5 py-1.5 rounded-lg font-bold hover:bg-indigo-100 transition-colors shadow-sm"><i className="fa-solid fa-plus mr-1"></i> Custom</button>
                            <input type="file" ref={stickerInputRef} accept="image/*" className="hidden" onChange={handleAddSticker} />
                          </div>
                          {customStickers.length === 0 ? <p className="text-xs text-slate-400 p-4 text-center w-full bg-slate-50 rounded-xl border border-dashed border-slate-200">No custom stickers added. Click + Custom to upload.</p> : customStickers.map((src, i) => (
                            <img key={i} src={src} onClick={() => sendSticker(src)} className="w-14 h-14 object-cover cursor-pointer hover:scale-110 transition-transform bg-slate-50 rounded-xl p-1.5 border border-slate-200 shadow-sm" />
                          ))}
                        </div>
                      )}
                      ...
                      {pastedImage && (
                        <div className="bg-white border border-indigo-200 rounded-xl p-3 flex items-center justify-between w-[95%] shadow-lg pointer-events-auto mb-2 animate-in slide-in-from-bottom-2 border-l-4 border-l-indigo-500">
                          <div className="flex items-center gap-4">
                            <img src={URL.createObjectURL(pastedImage)} className="w-12 h-12 object-cover rounded-lg border border-slate-200 shadow-sm" />
                            <div className="flex flex-col">
                              <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Clipboard Attachment</span>
                              <span className="text-xs font-bold text-slate-600">Image ready to send</span>
                            </div>
                          </div>
                          <button onClick={removePastedImage} className="text-slate-400 hover:text-rose-500 w-8 h-8 rounded-lg hover:bg-slate-50 flex items-center justify-center transition-colors"><i className="fa-solid fa-xmark"></i></button>
                        </div>
                      )}
                      ...
                      {replyingToMessage && (
                        <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center justify-between w-[95%] shadow-lg pointer-events-auto mb-2 animate-in slide-in-from-bottom-2 border-l-4 border-l-indigo-500">
                          <div className="flex flex-col pl-2 border-l border-slate-100">
                            <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-1"><i className="fa-solid fa-reply mr-1"></i> Replying to {replyingToMessage.profiles?.full_name || "Message"}</span>
                            <span className="text-xs text-slate-600 font-medium truncate w-[300px] sm:w-[500px]">{replyingToMessage.message || "Media Attachment"}</span>
                          </div>
                          <button onClick={() => setReplyingToMessage(null)} className="text-slate-400 hover:text-rose-500 w-8 h-8 rounded-lg hover:bg-slate-50 flex items-center justify-center transition-colors"><i className="fa-solid fa-xmark"></i></button>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-4 border-t border-slate-200 bg-white z-10 relative">
                      {editingMessage && (
                        <div className="absolute -top-8 left-4 bg-amber-50 border border-amber-200 text-amber-800 text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-t-xl flex items-center gap-2 shadow-sm">
                          <i className="fa-solid fa-pen"></i> Editing Message <button onClick={() => { setEditingMessage(null); setChatInput(""); }} className="ml-3 hover:text-rose-600 transition-colors"><i className="fa-solid fa-xmark"></i></button>
                        </div>
                      )}
                      ...
                      <div className="flex items-end gap-3 bg-slate-50 border border-slate-200 rounded-2xl p-2 pr-3 focus-within:border-indigo-400 focus-within:bg-white transition-all shadow-sm">
                        <button onClick={() => setShowStickerPicker(!showStickerPicker)} className={`w-10 h-10 flex items-center justify-center rounded-xl transition-colors shrink-0 mb-0.5 border ${showStickerPicker ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-white text-slate-400 hover:text-indigo-600 hover:bg-slate-100 border-slate-200 shadow-sm'}`} title="Stickers">
                        ...
                        <button onClick={() => chatMediaInputRef.current.click()} disabled={isSendingChat} className="w-10 h-10 flex items-center justify-center rounded-xl text-slate-400 hover:text-indigo-600 hover:bg-slate-100 transition-colors shrink-0 mb-0.5 shadow-sm bg-white border border-slate-200" title="Upload Image/Video">
                        ...
                        <textarea 
                          ...
                          placeholder={editingMessage ? "Edit your message..." : `Message... (Ctrl+V to paste images)`} 
                          value={chatInput} 
                        ...
                        <button onClick={handleSendChatMessage} disabled={ isSendingChat || ((!chatInput.trim() && !pastedImage) && !isSendingChat) } className="w-10 h-10 flex items-center justify-center bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all shrink-0 disabled:opacity-50 disabled:cursor-not-allowed mb-0.5 shadow-sm">
              ...
              {activeTab === "staff" && userProfile.role === "admin" && (
                <div className="h-full flex flex-col space-y-6 animate-in fade-in duration-500 w-full">
                  <div>
                    <h1 className="text-[32px] font-black text-slate-900 tracking-tight">Staff Directory</h1>
                    <p className="text-slate-500 text-sm mt-1 font-medium">Comprehensive registry of all corporate personnel.</p>
                  </div>
                  ...
                  <div className="bg-white rounded-[24px] shadow-sm border border-slate-200 p-4 flex flex-col md:flex-row gap-4 items-center w-full">
                    <div className="flex-1 w-full">
                      <div className="relative">
                        ...
                        <input type="text" placeholder="Search by Name or Staff ID..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-900 focus:ring-2 focus:ring-indigo-500 outline-none transition-all shadow-sm" />
                      </div>
                    </div>
                    
                    <div className="flex gap-4 w-full md:w-auto">
                      <select className="bg-slate-50 text-slate-900 border border-slate-200 text-sm font-bold rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer shadow-sm" value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)}>
                        <option value="All">All Roles</option>
                        <option value="admin">Admin</option>
                        <option value="team_lead">Team Lead</option>
                        <option value="ai_engineer">AI Engineer</option>
                      </select>
                      <select className="bg-slate-50 text-slate-900 border border-slate-200 text-sm font-bold rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer shadow-sm" value={teamFilter} onChange={(e) => setTeamFilter(e.target.value)}>
                        ...
                        {filteredStaff.length === 0 ? (
                          <tr>
                            <td colSpan="5" className="px-6 py-16 text-center text-slate-400">
                              ...
                              <p className="font-bold text-lg text-slate-600">No personnel found</p>
                            </td>
                          </tr>
                        ) : (
                          filteredStaff.map((staff) => (
                            <tr key={staff.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                              <td className="px-6 py-4 flex items-center gap-4">
                                ...
                                <div>
                                  <span className="font-extrabold text-slate-900 block">{staff.full_name}</span>
                                  <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{staff.role.replace("_", " ")}</span>
                                </div>
                              </td>
                              <td className="px-6 py-4 font-mono text-slate-500 text-xs font-bold">{staff.staff_id}</td>
                              <td className="px-6 py-4">
                                <span className={`text-xs font-bold truncate max-w-[200px] block ${staff.current_task === 'Idle / Monitored' ? 'text-slate-400' : 'text-slate-900'}`}>{staff.current_task}</span>
                              </td>
                              <td className="px-6 py-4">
                              ...
                              <td className="px-6 py-4 text-right">
                                <div className="flex justify-end gap-2">
                                  <button onClick={() => handleViewStaffTasks(staff.id, staff.full_name, staff.staff_id)} className="bg-white border border-slate-200 text-slate-700 hover:border-slate-300 hover:bg-slate-50 px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm">
                                    <i className="fa-solid fa-list-check mr-1"></i> Tasks
                                  </button>
                                  {staff.id !== userProfile.id && (
                                    <button onClick={() => handleBanStaff(staff)} className={`${staff.ban_status !== 'none' ? 'bg-emerald-50 border-emerald-100 text-emerald-700 hover:bg-emerald-100' : 'bg-rose-50 border-rose-100 text-rose-700 hover:bg-rose-100'} border px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm`}>
              ...
              {activeTab === "departments" && userProfile.role === "admin" && (
                <div className="h-full flex flex-col space-y-6 animate-in fade-in duration-500 w-full">
                  <div>
                    <h1 className="text-[32px] font-black text-slate-900 tracking-tight">Departments & Teams</h1>
                    <p className="text-slate-500 text-sm mt-1 font-medium">Overview of all team leads, their divisions, and deployed operatives.</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
                    {allTeamsData.map((team) => (
                      <div key={team.id} className="bg-white rounded-[24px] shadow-sm hover:shadow-md border border-slate-200 p-8 flex flex-col justify-between transition-shadow">
                        <div>
                          <div className="flex items-center justify-between mb-6 border-b border-slate-100 pb-4">
                            <h3 className="text-xl font-black text-slate-900">{team.name}</h3>
                            <span className="px-3 py-1 bg-emerald-50 border border-emerald-100 text-emerald-700 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm">Active</span>
                          </div>
                          <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold mb-2">Team Lead</p>
                          ...
                          <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold mb-3">Assigned Operatives</p>
                          <ul className="space-y-2 mb-4">
                            {team.team_members && team.team_members.length > 0 ? (
                              team.team_members.map((tm) => (
                                <li key={tm.user_id} className="text-xs font-bold text-slate-700 bg-white border border-slate-200 px-4 py-2.5 rounded-xl flex items-center justify-between shadow-sm">
                                  <span>{tm.profiles?.full_name}</span>
                                  <span className="text-[9px] uppercase bg-slate-100 border border-slate-200 text-slate-500 px-2 py-1 rounded-md font-black">{tm.profiles?.role.replace("_", " ")}</span>
                                </li>
                              ))
                            ) : ( <li className="text-xs font-semibold text-slate-400 italic bg-slate-50 border border-slate-200 p-4 rounded-xl text-center shadow-sm">No members deployed in this division yet.</li> )}
                          </ul>
                        </div>
                      </div>
                    ))}
              ...
              {activeTab === "reports" && (
                <div className="h-full flex flex-col space-y-6 animate-in fade-in duration-500 w-full">
                  <div>
                    <h1 className="text-[32px] font-black text-slate-900 tracking-tight">Operational Reports</h1>
                    <p className="text-slate-500 text-sm mt-1 font-medium">{userProfile.role === "admin" ? "Review and manage bi-weekly reports submitted by divisions." : "Upload and track your division's bi-weekly performance reports."}</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1 min-h-0 w-full">
                    {userProfile.role === "team_lead" && (
                      <div className="lg:col-span-1 bg-white rounded-[24px] shadow-sm hover:shadow-md border border-slate-200 p-8 flex flex-col transition-all">
                        <h3 className="text-lg font-black text-slate-900 mb-6 border-b border-slate-100 pb-4">Upload New Report</h3>
                        <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-slate-300 hover:border-indigo-400 rounded-[20px] bg-slate-50 transition-colors p-8 text-center cursor-pointer" onClick={() => fileInputRef.current.click()}>
                          <i className="fa-solid fa-cloud-arrow-up text-5xl text-indigo-500 mb-4"></i>
                          <p className="text-sm font-bold text-slate-900 mb-1">Click to Upload PDF</p>
                          <p className="text-xs font-semibold text-slate-500 mb-6">Max file size: 10MB</p>
                          <input type="file" ref={fileInputRef} accept="application/pdf" className="hidden" onChange={handleFileUpload} disabled={isUploading} />
                          <button disabled={isUploading} className={`bg-indigo-600 text-white px-6 py-2.5 rounded-xl text-xs font-bold transition-all shadow-sm ${isUploading ? "opacity-50 cursor-not-allowed" : "hover:bg-indigo-700"}`}>
                    ...
                    <div className={`${userProfile.role === "team_lead" ? "lg:col-span-2" : "lg:col-span-3"} bg-white rounded-[24px] shadow-sm hover:shadow-md border border-slate-200 overflow-hidden flex flex-col transition-all w-full`}>
                      <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-white">
                        <h3 className="text-lg font-black text-slate-900">Submitted Reports Registry</h3>
                      </div>
                      <div className="overflow-x-auto custom-scrollbar flex-1 bg-white">
                        {reports.length === 0 ? (
                          <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-center p-8 bg-slate-50">
                            ...
                            <p className="text-lg font-bold text-slate-600">No reports found in the registry.</p>
                          </div>
                        ) : (
                                ...
                                <tr key={report.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                  <td className="px-6 py-4">
                                    <a href={report.file_url} target="_blank" rel="noopener noreferrer" className="font-bold text-indigo-600 hover:text-indigo-800 flex items-center transition-colors">
                                      <div className="w-8 h-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-rose-500 mr-3 shadow-sm"><i className="fa-solid fa-file-pdf"></i></div>
                                      {report.file_name}
                                    </a>
                                    <div className="text-[10px] font-bold text-slate-400 mt-1.5 ml-11">{new Date(report.created_at).toLocaleString()}</div>
                                  </td>
                                  <td className="px-6 py-4">
                                    <span className="px-3 py-1.5 bg-white border border-slate-200 text-slate-600 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm">{report.teams?.name || "Unknown Division"}</span>
                                  </td>
                                  <td className="px-6 py-4 text-slate-900 font-bold">{report.profiles?.full_name}</td>
                                  <td className="px-6 py-4">
                                    <span className={`px-3 py-1.5 rounded-lg border text-[10px] font-black uppercase tracking-widest shadow-sm ${report.status === "pending_approval" ? "bg-amber-50 border-amber-200 text-amber-700" : report.status === "approved" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-rose-50 border-rose-200 text-rose-700"}`}>
                                      {report.status.replace("_", " ")}
                                    </span>
                                    {report.status === "rejected" && ( <p className="text-[10px] text-rose-600 mt-2 font-bold max-w-xs leading-snug">Note: {report.admin_feedback}</p> )}
                                  </td>
                                  <td className="px-6 py-4 text-right">
                                    <div className="flex justify-end items-center gap-3">
                                      <a href={report.file_url} target="_blank" rel="noopener noreferrer" className="text-slate-500 hover:text-indigo-600 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm transition-colors font-bold text-xs" title="View PDF">
                                        <i className="fa-solid fa-eye mr-1"></i> View
                                      </a>
                                      {userProfile.role === "admin" && report.status === "pending_approval" && (
                                        <>
                                          <button onClick={() => handleApproveReport(report.id, report.profiles?.full_name, report.teams?.name, report.lead_id)} className="bg-white border border-slate-200 text-emerald-600 hover:bg-emerald-50 hover:border-emerald-300 w-8 h-8 rounded-lg shadow-sm transition-colors" title="Approve"><i className="fa-solid fa-check"></i></button>
                                          <button onClick={() => handleRejectReport(report.id, report.profiles?.full_name, report.teams?.name, report.lead_id)} className="bg-white border border-slate-200 text-rose-600 hover:bg-rose-50 hover:border-rose-300 w-8 h-8 rounded-lg shadow-sm transition-colors" title="Reject"><i className="fa-solid fa-xmark"></i></button>
                                        </>
                                      )}
                                    </div>
                                  </td>
              ...
              {activeTab === "tasks" && (
                <div className="h-full flex flex-col animate-in fade-in duration-500 w-full">
                  <div className="flex justify-between items-end mb-8 w-full">
                    <div>
                      <h1 className="text-[32px] font-black text-slate-900 tracking-tight">Task Manager</h1>
                      <p className="text-slate-500 text-sm mt-1 font-medium">{userProfile.role === "admin" ? "Dispatch and oversee global operational directives." : "Manage assigned directives. Golden tasks are high priority."}</p>
                    </div>
                    {userProfile.role === "admin" && (
                      <button onClick={handleAdminDispatchDirective} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl text-sm font-black shadow-sm transition-all flex items-center tracking-wide uppercase">
                        <i className="fa-solid fa-bolt mr-2 text-amber-300"></i> Notify All Divisions
                      </button>
                    )}
                  </div>
                  <div className="bg-white rounded-[24px] shadow-sm border border-slate-200 overflow-x-auto flex-1 w-full">
                    {loadingTasks ? (
                      <div className="flex items-center justify-center h-64 text-indigo-600"><i className="fa-solid fa-circle-notch fa-spin text-4xl"></i></div>
                    ) : tasks.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-center bg-slate-50">
                        ...
                        <h3 className="text-2xl font-black text-slate-900">Queue Cleared</h3>
                        <p className="text-sm font-medium text-slate-500 mt-2">All operational directives have been processed.</p>
                      </div>
                    ) : (
                      <table className="w-full text-left border-collapse min-w-[800px]">
                        ...
                        <tbody className="text-sm font-medium">
                          {tasks.map((task) => (
                            <tr key={task.id} className={`border-b transition-colors ${task.is_admin_directive ? "bg-amber-50/50 hover:bg-amber-50 border-l-4 border-l-amber-400 border-b-slate-100" : "border-slate-100 hover:bg-slate-50 border-l-4 border-l-transparent"}`}>
                              <td className="px-6 py-5">
                                <div className={`font-black text-base tracking-tight ${task.is_admin_directive ? "text-amber-700" : "text-slate-900"}`}>
                                  {task.is_admin_directive && ( <i className="fa-solid fa-star text-amber-500 mr-2 text-sm drop-shadow-sm"></i> )}
                                  {task.title}
                                </div>
                                {task.file_url && (
                                  <a href={task.file_url} target="_blank" rel="noopener noreferrer" className="text-[11px] text-slate-600 hover:text-indigo-600 mt-2 flex items-center font-bold w-fit bg-white border border-slate-200 px-2.5 py-1 rounded-md transition-colors shadow-sm">
                                    <i className="fa-solid fa-file-pdf text-rose-500 mr-1.5"></i> View Directive PDF
                                  </a>
                                )}
                              </td>
                              <td className="px-6 py-5">
                                <span className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest ${getDivisionStyle(task.team)} border shadow-sm`}>
                                  {task.team}
                                </span>
                              </td>
                              <td className="px-6 py-5 text-slate-900 font-bold">
                                <div className="flex items-center gap-3">
                                  <div className="w-8 h-8 rounded-full bg-white border border-slate-200 shadow-sm flex items-center justify-center text-[10px] text-slate-700 font-bold">{task.assignedToName.charAt(0)}</div>
                                  {task.assignedToName}
                                </div>
                              </td>
                              <td className="px-6 py-5">
                                <span className={`px-3 py-1.5 border rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm ${task.status === "in_progress" ? "bg-indigo-50 border-indigo-200 text-indigo-700" : task.status === "pending_completion_approval" ? "bg-purple-50 border-purple-200 text-purple-700" : task.status === "rejected" ? "bg-rose-50 border-rose-200 text-rose-700" : "bg-emerald-50 border-emerald-200 text-emerald-700"}`}>
                                  {task.status.replace(/_/g, " ")}
                                </span>
                                {task.status === "rejected" && ( <p className="text-[10px] text-rose-600 mt-2 font-bold max-w-[200px] leading-tight">Reason: {task.adminFeedback}</p> )}
                              </td>
                              <td className="px-6 py-5 text-right">
                                {userProfile.role === "admin" && task.status === "pending_completion_approval" ? (
                                  <div className="flex justify-end gap-2">
                                    <button onClick={() => handleApproveCompletion(task.id) } className="bg-white border border-slate-200 text-emerald-600 hover:bg-emerald-50 hover:border-emerald-200 w-9 h-9 rounded-xl shadow-sm transition-colors font-bold text-sm" title="Approve">
                                      <i className="fa-solid fa-check"></i>
                                    </button>
                                    <button onClick={() => handleRejectCompletion(task.id) } className="bg-white border border-slate-200 text-rose-600 hover:bg-rose-50 hover:border-rose-200 w-9 h-9 rounded-xl shadow-sm transition-colors font-bold text-sm" title="Reject">
                                      <i className="fa-solid fa-xmark"></i>
                                    </button>
                                  </div>
                                ) : userProfile.role === "ai_engineer" && task.status === "in_progress" ? (
                                  <button onClick={() => handleEngineerMarkComplete(task.id, task.title, task.assignedToName, task.team_id, task.assigned_to) } className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold text-xs shadow-sm transition-all">Mark Complete</button>
                                ) : (
                                  <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Monitored</span>
                                )}
                              </td>
                            </tr>
                          ))}
              ...
              {activeTab === "activity-log" && userProfile.role === "admin" && (
                <div className="h-full flex flex-col animate-in fade-in duration-500 w-full">
                  <div className="flex justify-between items-end mb-8 w-full">
                    <div>
                      <h1 className="text-[32px] font-black text-slate-900 tracking-tight">System Activity Log</h1>
                      <p className="text-slate-500 text-sm mt-1 font-medium">Real-time immutable audit log of system actions.</p>
                    </div>
                  </div>
                    ...
                    <div className="overflow-x-auto custom-scrollbar">
                      <div className="min-w-[800px]">
                        ...
                        <div className="p-3 space-y-1 font-mono text-xs">
                          {logs.map((log) => (
                            <div key={log.id} className="grid grid-cols-12 px-4 py-3 rounded-xl hover:bg-slate-50 border-l-4 border-transparent hover:border-slate-300 transition-colors cursor-default">
                              <div className="col-span-3 text-slate-500 font-semibold tracking-tight">[{new Date(log.created_at).toLocaleString([], { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' }) }]</div>
                              <div className="col-span-3 text-slate-900 font-bold">{log.actor_name} <span className="text-[9px] uppercase tracking-widest text-slate-500 ml-1 bg-white border border-slate-200 shadow-sm px-1.5 py-0.5 rounded">{log.actor_role.replace("_", " ")}</span></div>
                              <div className="col-span-6 text-slate-700 font-medium">{log.action_description}</div>
                            </div>
                          ))}
                        </div>
                      </div>
              ...
              {["ai-agents", "clients"].includes(activeTab) && (
                <div className="h-full animate-in fade-in duration-500 w-full">
                  <h1 className="text-[32px] font-black text-slate-900 mb-8 capitalize tracking-tight">{activeTab.replace("-", " ")}</h1>
                  <div className="bg-white rounded-[24px] shadow-sm border border-slate-200 p-8 text-center h-[60vh] flex flex-col justify-center items-center">
                    ...
                    <h2 className="text-2xl font-black text-slate-900">Registry Module Active</h2>
                    <p className="text-slate-500 mt-2 font-medium">Secure database connection established. Telemetry standing by.</p>
                  </div>
                </div>
              )}
            </div>
