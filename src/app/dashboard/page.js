...
export default function ZenTechDashboard() {
  ...
  const fetchUserChannels = async (profile) => {
    ...
    if (profile.role === 'admin') {
      channels = [
        { id: 'Admin', label: 'Admin (Leadership Network)' },
      ...
      channels.push({ id: 'Admin', label: 'Admin (Leadership Network)' });
  ...
  const fetchChatMessages = async () => {
    ...
    const { data, error } = await supabase
      .from('chats')
      .select('id, message, media_url, media_type, created_at, profiles:sender_id(full_name, role)')
  ...
  const handleAdminDispatchDirective = async () => {
    const { value: formValues } = await Swal.fire({
      title: 'Dispatch Golden Directive',
      html: `
  ...
  const fetchTasks = async () => {
    ...
    if (!error && data) {
      const formattedTasks = data.map(task => ({
        id: task.id, title: task.title, status: task.status, team_id: task.team_id, 
        is_admin_directive: task.is_admin_directive, file_url: task.file_url,
        assigned_to: task.assigned_to, team: task.teams?.name || 'Assigned Operations', 
        assignedToName: task.profiles?.full_name || 'Unassigned (Team Pool)',
        assignedByName: task.assigned_by_name || 'Team Lead', adminFeedback: task.admin_feedback || 'None'
      }));
  ...
  const handleFileUpload = async (event) => {
    ...
    if (file.type !== 'application/pdf') {
      Swal.fire('Invalid Format', 'Only PDF files are allowed for reports.', 'error');
  ...
  const handleEngineerMarkComplete = async (taskId, taskTitle, assignedToName, teamIdVal, assignedToId) => {
    ...
    if (!error) {
      Swal.fire('Submitted!', 'Task marked as complete and sent to Admin for final approval.', 'success');
      fetchTasks();
    }
  };
          ...
          <div className="h-full overflow-y-auto overflow-x-hidden flex flex-col custom-scrollbar">
            ...
            <div className="flex-1 px-4 py-6 space-y-6">
              ...
              <div>
                ...
                <ul className="space-y-1">
                  <NavButton id="chat" icon="fa-solid fa-comments" label="Comms Network" allowedRoles={['admin', 'team_lead', 'ai_engineer']} />
                </ul>
              </div>
              ...
              <div>
                ...
                <ul className="space-y-1">
                  {userProfile.role === 'ai_engineer' && <NavButton id="tasks" icon="fa-solid fa-code" label="My Active Tasks" allowedRoles={['ai_engineer']} />}
          ...
          <header className="bg-white h-[4.5rem] border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 flex-shrink-0 z-10 w-full">
            <div className="flex items-center gap-4">
              <span className="inline-block px-3 py-1 bg-slate-800 text-white rounded-md text-[0.65rem] font-bold uppercase tracking-widest">
                {userProfile.role.replace('_', ' ')} PRIVILEGES
              </span>
            </div>
            <div className="flex items-center gap-4 sm:gap-6">
              <span className="hidden sm:inline-block text-xs font-bold text-slate-700">{userProfile.full_name}</span>
            </div>
          </header>
            ...
            <div className="max-w-[1600px] mx-auto w-full h-full">
                ...
                <div className="h-full flex gap-6 pb-2">
                  ...
                  <div className="w-72 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden flex-shrink-0">
                     <div className="p-5 border-b border-slate-100 bg-slate-50 flex items-center gap-3">
                        ...
                        <h2 className="font-bold text-slate-800 text-lg">Comms Network</h2>
                     </div>
                     <div className="flex-1 overflow-y-auto p-3 space-y-1 custom-scrollbar">
                       <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 px-2">Accessible Channels</p>
                         ...
                         chatMessages.map(msg => {
                           ...
                           return (
                             <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                               ...
                               <div className={`px-4 py-2.5 rounded-2xl max-w-[75%] shadow-sm text-sm ${isMe ? 'bg-blue-600 text-white rounded-tr-sm' : 'bg-white border border-slate-200 text-slate-800 rounded-tl-sm'}`}>
                                 {msg.media_url && msg.media_type === 'image' && (
                                   <img src={msg.media_url} alt="Chat Upload" className="max-w-full h-auto rounded-lg mb-2 border border-black/10" style={{ maxHeight: '300px' }} />
                                 )}
                                 {msg.media_url && msg.media_type === 'video' && (
                                   <video src={msg.media_url} controls className="max-w-full h-auto rounded-lg mb-2 border border-black/10" style={{ maxHeight: '300px' }} />
                                 )}
                    ...
                    <div className="p-4 border-t border-slate-200 bg-white">
                       <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl p-2 pr-3 focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 transition-all">
                          ...
                          <button 
                            ...
                            disabled={isSendingChat || (!chatInput.trim() && !isSendingChat)}
              ...
              {activeTab === 'dashboard' && (
                <div className="space-y-6">
                  <div>
                    <h1 className="text-2xl font-bold text-slate-800">Dashboard Telemetry</h1>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 flex flex-col items-center justify-center">
                      <h3 className="text-lg font-bold text-slate-800 mb-6 w-full text-left">
                        {userProfile.role === 'admin' ? 'Division Task Distribution' : 'Team Task Success Rate'}
                      </h3>
                ...
                <div className="h-full flex flex-col space-y-6">
                  <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">
                      {userProfile.role === 'admin' ? 'Allocate unassigned AI Engineers to teams (Payal, Sushant, Pratik).' : 'Your designated team members.'}
                    </p>
                  </div>
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                      ...
                      {unassignedEngineers.length === 0 ? (
                        <div className="text-slate-400 text-sm py-4 text-center border border-dashed border-slate-200 rounded-lg">
                          No new member has been added. All engineers are deployed.
                        </div>
                      ) : (
                            ...
                            {unassignedEngineers.map(eng => (
                              <tr key={eng.id} className="border-b border-slate-100">
                                ...
                                <td className="px-4 py-3 text-right">
                                  <button onClick={() => handleAssignToTeam(eng.id, eng.full_name)} className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-xs font-medium">
                                    Assign to Team
                                  </button>
                                </td>
                              </tr>
              ...
              {activeTab === 'reports' && (
                <div className="h-full flex flex-col space-y-6">
                  <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">
                      {userProfile.role === 'admin' ? 'Review and manage bi-weekly reports submitted by divisions.' : 'Upload and track your division\'s bi-weekly performance reports.'}
                    </p>
                  </div>
                            ...
                            <tbody className="text-sm">
                              {reports.map(report => (
                                <tr key={report.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                  ...
                                  <td className="px-6 py-4">
                                    <span className={`px-2 py-1 rounded text-[0.65rem] font-bold uppercase ${
                                      report.status === 'pending_approval' ? 'bg-amber-100 text-amber-700' :
                                    ...
                                    {report.status === 'rejected' && (
                                      <p className="text-[0.65rem] text-red-500 mt-1 font-bold">Note: {report.admin_feedback}</p>
                                    )}
                                  </td>
                                  <td className="px-6 py-4 text-right">
                                    <div className="flex justify-end items-center gap-2">
                                      ...
                                      {userProfile.role === 'admin' && report.status === 'pending_approval' && (
                                        <>
                                          <button onClick={() => handleApproveReport(report.id, report.profiles?.full_name, report.teams?.name, report.lead_id)} className="bg-green-600 hover:bg-green-700 text-white px-2 py-1 rounded text-xs shadow-sm" title="Approve">
                                            <i className="fa-solid fa-check"></i>
                                          </button>
                                          <button onClick={() => handleRejectReport(report.id, report.profiles?.full_name, report.teams?.name, report.lead_id)} className="bg-red-600 hover:bg-red-700 text-white px-2 py-1 rounded text-xs shadow-sm" title="Reject">
              ...
              {activeTab === 'tasks' && (
                <div className="h-full flex flex-col">
                  <div className="flex justify-between items-end mb-6">
                    <div>
                      <h1 className="text-2xl font-bold text-slate-800">Task Master Console</h1>
                      <p className="text-slate-500 text-sm mt-1">
                        {userProfile.role === 'admin' ? 'Dispatch golden directives and monitor division tasks.' : 'Manage assigned directives. Golden tasks are high priority.'}
                      </p>
                    </div>
                    {userProfile.role === 'admin' && (
                      <button onClick={handleAdminDispatchDirective} className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-md transition-colors flex items-center">
                        <i className="fa-solid fa-bolt mr-2"></i> Dispatch Admin Directive
                      </button>
                    )}
                  </div>
                            ...
                            <tr key={task.id} className={`border-b transition-colors ${task.is_admin_directive ? 'bg-yellow-50 hover:bg-yellow-100 border-l-4 border-l-yellow-400' : 'border-slate-100 hover:bg-slate-50'}`}>
                              ...
                              <td className="px-6 py-4">
                                <span className={`px-2 py-1 rounded text-[0.65rem] font-bold uppercase ${
                                  ...
                                  task.status === 'pending_completion_approval' ? 'bg-purple-100 text-purple-700' :
                                  task.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                                }`}>
                              ...
                              <td className="px-6 py-4 text-right">
                                {userProfile.role === 'admin' && task.status === 'pending_completion_approval' ? (
                                  <div className="flex justify-end gap-2">
                                    <button onClick={() => handleApproveCompletion(task.id)} className="bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded text-xs font-medium shadow-sm"><i className="fa-solid fa-check mr-1"></i> Approve</button>
                                    <button onClick={() => handleRejectCompletion(task.id)} className="bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded text-xs font-medium shadow-sm"><i className="fa-solid fa-xmark mr-1"></i> Reject</button>
                                  </div>
                        ...
                        <div className="p-2 space-y-1 font-mono text-sm">
                          {logs.map((log) => (
                            <div key={log.id} className="grid grid-cols-12 p-2 rounded hover:bg-slate-50 border-l-2 border-transparent hover:border-slate-400 transition-colors">
                              <div className="col-span-3 text-slate-400 text-xs">[{new Date(log.created_at).toLocaleTimeString()}]</div>
